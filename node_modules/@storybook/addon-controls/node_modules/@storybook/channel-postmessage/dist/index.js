  ...
  _createClass(PostmsgTransport, [{
    ...
    value: function send(event, options) {
      ...
      var data = (0, _telejson.stringify)({
        key: KEY,
        event: event,
        source: _global.document.location.origin + _global.document.location.pathname
      }, {
        maxDepth: depth,
    ...
    value: function handleEvent(rawEvent) {
      try {
        ...
        if (key === KEY) {
          ...
          if (source) {
            var _URL = new URL(source, _global.document.location),
                origin = _URL.origin,
          ...
          _clientLogger.pretty.debug.apply(_clientLogger.pretty, [_global.location.origin !== event.source ? "".concat(pageString, " received ").concat(eventString) : "".concat(pageString, " received ").concat(eventString, " <span style=\"color: gray\">(on ").concat(_global.location.origin, " from ").concat(event.source, ")</span>")].concat(_toConsumableArray(event.args)));
...
var getEventSourceUrl = function getEventSourceUrl(event) {
  ...
  if (frame && remainder.length === 0) {
    ...
    var _URL3 = new URL(src, _global.document.location),
        origin = _URL3.origin,
